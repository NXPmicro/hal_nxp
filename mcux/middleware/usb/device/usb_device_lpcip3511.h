 ...
 * Definitions
 ******************************************************************************/
/* IP3511 data buffer alignment */
//#define USB_DATA_ALIGN_SIZE 64
#define USB_DATA_ALIGN_SIZE_MULTIPLE(n) ((n + USB_DATA_ALIGN_SIZE - 1) & (~(USB_DATA_ALIGN_SIZE - 1)))

/*! @brief Disable the data and control transfer buffer memory copy,
 * application need make sure the buffer satisfy this controller.
 * ((not 64 bytes alignment) || (not in the same 64K ram) || (HS && OUT && not multiple of 4))
 */
#define USB_DEVICE_IP3511_DISABLE_ALL_TRANSFER_BUFFER_COPY (1U)
...
#else
#define USB_DEVICE_IP3511_ENDPOINT_RESERVED_BUFFER_SIZE (0u)
#define USB_DEVICE_IP3511_BITS_FOR_RESERVED_BUFFER (0u)
#define USB_DEVICE_IP3511_RESERVED_BUFFER_FOR_COPY (0u)
#endif

#if defined (CONFIG_USB_DEDICATED_MEMORY)
#define USB_DEVICE_IP3511_RESERVED_BUFFER_FOR_COPY (1u)
#else
#define USB_DEVICE_IP3511_RESERVED_BUFFER_FOR_COPY (0u)
#endif
